...
class database {
	...
	*/
	...
	/**
	* tells the db which connection to use
	* setActiveConnection($id) allows us to change this
	*/
	
	private $activeConneciton = 0;
	...
	*/
	...
	{
		$this->conneciton[] = new mysqli( $host, $user, $password, $database );
		$connection_id = count( $this->connection ) -1;
	...
	*/
	...
	/**
	* store a query in the query cache for the processing later
	...
	* @param int the query cach pointer
	...
	* @return int the pointerd to the array in the data cache
	*/
	...
	*/
     ...
     */
	 ...
	 {
	 	if( !$result = $this->connection[$this->activeConneciton]->query( $queryStr ) )
		{
     ...
     */
	 ...
	 {
		 return $this->$this->connection[$this->activeConnection]->affected_rows;
	 }
